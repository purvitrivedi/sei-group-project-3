...
userSchema.virtual("eventsGoing", {
  ref: "Group",
  localField: "_id",
  foreignField: "events.participants",
});

userSchema.virtual("imagesAddedtoHike", {
  ref: "Hike",
  localField: "_id",
...
userSchema.virtual("createdHikes", {
  ref: "Hike",
  localField: "_id",
  foreignField: "user",
});
...
userSchema.pre("validate", function (next) {
  if (
    ...
    this._passwordConfimation !== this.password
  ) {
...
userSchema.pre("save", function (next) {
  // * <--- will run before the model is saved
