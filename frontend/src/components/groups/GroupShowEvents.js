  ...
  return (
    ...
    >
      {events.map((item) => {
        ...
        const participantId = item.participants.find(
          (par) => par.user._id === userId
        );
        return (
          <div className="box" key={item._id}>
            {item.createdMember._id === getUserId() && (
              <div className="buttons is-right">
                <Link
                  to={`/groups/${group._id}/events/${item._id}/edit`}
            ...
            <h1
              className="subtitle"
              style={{ fontSize: 30, fontFamily: "Amatic SC, cursive" }}
            >
            ...
            <div className="columns">
              {item.hike && (
                <div className="column">
                  <p style={{ fontFamily: "Amatic SC, cursive", fontSize: 20 }}>
                    <i className="fas fa-mountain"></i>&nbsp; Course:&nbsp;
                    {item.hike.name}&nbsp;
                  </p>
                  ...
                  <p
                    className="HikeDescription"
                    style={{ fontSize: 15, maxHeight: 300, overflow: "auto" }}
                  >
                    &nbsp;&nbsp;&nbsp;
              ...
              <div className="column">
                <p style={{ fontSize: 20, fontFamily: "Amatic SC, cursive" }}>
                  <i className="fas fa-user"></i>&nbsp; Event Host:&nbsp;
                  {item.createdMember.username.replace(
                    item.createdMember.username[0],
                    item.createdMember.username[0].toUpperCase()
                  )}
                </p>
                ...
                <Link to={`/profiles/${item.createdMember._id}`}>
                  <figure className="image is-rounded is-64x64">
                    <img
                      ...
                      src={item.createdMember.profileImage}
                      alt={item.createdMember.username}
                    />
                  </figure>
                </Link>
                <br />
                {numOfPs > 1 ? (
                  ...
                  >{`${numOfPs} members will participate`}</p>
                ) : (
                ...
                <p>{`From ${item.startDate.slice(
                  0,
                  10
                )} to ${item.endDate.slice(0, 10)}`}</p>
                <div style={{ margin: 20 }}>
                  <Calendar
