...
class GroupShow extends React.Component {
  ...
  handleUnsubscribe = async () => {
    try {
      const groupId = this.props.match.params.id;
      const userId = getUserId();
      const memberToRemove = this.state.group.members.find(
  ...
  sendEmail = async (email) => {
    ...
    window.location.href =
      "mailto:" + email + "?body=" + body + "&subject=" + subject;
  };
  ...
  handleLikes = async (groupId, messageId, likes) => {
    try {
      const userId = getUserId();
      if (likes.length > 0 && likes.find((like) => like.user._id === userId))
        return;
      ...
      await axios.put(
        `/api/groups/${groupId}/messages/${messageId}/likes`,
        resUser.data,
        {
          headers: { Authorization: `Bearer ${getToken()}` },
        }
          ...
          <section className="group-show-buttons">
            <div className="buttons has-addons">
              ...
              {member && (
                <Link to={`/groups/${group._id}/events`}>
                  <button className="button is-success is-light  group-show-button">
            ...
            <div className="buttons has-addons">
              ...
              {isAuthenticated() && !member && (
                <button
                  className="button group-show-button is-info"
