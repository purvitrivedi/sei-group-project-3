...
class HikeShow extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      if (!loggedIn) {
        ...
        this.setState(
          { hike: res.data, user: user.data.favoritedHikes },
          () => {
            this.getAverageRating();
  ...
  handleRemoveFavorite = async () => {
    try {
      ...
      let favArr = this.state.user;
      favArr = favArr.map((item) => [item._id, item.hike._id]);
  ...
  handleSubmitReview = async (event, rating, text) => {
    ...
    try {
      ...
      const res = await getSingleHike(hikeId);
      this.setState(
        { hike: res.data, errors: "", reviewText: "", reviewRating: "" },
        () => {
          this.getAverageRating();
        }
      );
    ...
    return (
      <div className="HikeShow box">
        <div className="hero is-medium is-success">
          <div
            className="hero-body"
            style={{ backgroundImage: `url(${hike.images[0]})` }}
          >
        ...
        <div className="box">
          <section className="hike-info">
            ...
            <h1>
              Difficulty:{" "}
              {hike.difficulty.map((difficulty) => {
                return `${difficulty}, `;
              })}
            </h1>
            <h1>
              Seasons:{" "}
              {hike.seasons.map((season) => {
                return `${season}, `;
              })}
            </h1>
          ...
          <section className="hike-show-buttons">
            <div className="buttons has-addons">
              ...
              {isAuthenticated() && (
                <button
                  className="button hike-show-button is-primary is-light"
                  onClick={this.handleImageUploadActive}
                >
          ...
          <section>
            ...
            <section
              className={
                this.state.imageUploadActive
                  ? "image-upload"
                  : "image-upload is-hidden"
              }
            >
              <ImageUpload onChange={this.handleAddImage} />
            </section>
          </section>
          <hr />
          <div className="columns is-multiline">
            <div className="column is-half-desktop">
              ...
              <br />
              <HikeShowMap hike={this.state.hike} />
            </div>
            <div className="column is-half-desktop">
